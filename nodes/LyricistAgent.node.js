...
async function detectLanguage(text) {
    try {
        ...
        for (const [lang, words] of Object.entries(commonWords)) {
            const wordCount = words.filter(word => text.toLowerCase().includes(word)).length;
            if (wordCount > 0) {
                detectedLanguage = lang;
...
async function localizeContent(request) {
    try {
        ...
        const sourceLocale = localizations[request.sourceLocale] || localizations['en-US'];
        const targetLocale = localizations[request.targetLocale] || localizations['en-US'];
        let localizedContent = request.content;
        // Apply cultural adaptations
        if (sourceLocale.greeting !== targetLocale.greeting) {
            localizedContent = localizedContent.replace(new RegExp(sourceLocale.greeting, 'gi'), targetLocale.greeting);
        }
                ...
                switch (operation) {
                    case 'translate':
                        const request = {
                            ...
                            sourceLanguage: this.getNodeParameter('sourceLanguage', i),
                            targetLanguage: this.getNodeParameter('targetLanguage', i),
                        ...
                        break;
                    case 'detect':
                        const text = this.getNodeParameter('detectText', i);
                        result = await detectLanguage(text);
                        break;
                    case 'localize':
                        const localizationRequest = {
                            content: this.getNodeParameter('localizeContent', i),
                            sourceLocale: this.getNodeParameter('sourceLocale', i),
                            targetLocale: this.getNodeParameter('targetLocale', i),
...
exports.LyricistAgent = LyricistAgent;
