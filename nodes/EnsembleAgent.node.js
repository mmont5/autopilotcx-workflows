...
async function escalateIssue(request) {
    try {
        ...
        const allowedTargets = validEscalations[request.currentLevel] || [];
...
async function trackResolution(tracking) {
    try {
        ...
        const statusConfig = statusUpdates[tracking.status];
        ...
        const progress = progressMap[tracking.status] || 0;
                ...
                switch (operation) {
                    case 'support':
                        const supportRequest = {
                            ...
                            contactInfo: {
                                name: this.getNodeParameter('customerName', i),
                                email: this.getNodeParameter('customerEmail', i),
                    ...
                    case 'escalate':
                        const escalationRequest = {
                            supportTicketId: this.getNodeParameter('supportTicketId', i),
                            reason: this.getNodeParameter('escalationReason', i),
                            currentLevel: this.getNodeParameter('currentLevel', i),
                            targetLevel: this.getNodeParameter('targetLevel', i),
                            description: this.getNodeParameter('description', i),
                            urgency: this.getNodeParameter('escalationUrgency', i),
                        };
                        result = await escalateIssue(escalationRequest);
                        break;
                    case 'track':
                        const trackingRequest = {
                            ticketId: this.getNodeParameter('supportTicketId', i),
                            status: this.getNodeParameter('resolutionStatus', i),
                            assignedTo: this.getNodeParameter('assignedTo', i),
                            estimatedResolution: this.getNodeParameter('estimatedResolution', i),
                            notes: [this.getNodeParameter('resolutionNotes', i)].filter(Boolean),
                        };
