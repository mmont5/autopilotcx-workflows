...
class BookingAgent {
    ...
    async execute() {
        ...
        const getRandomVariation = (category) => {
        ...
        const formatResponse = (template, data = {}) => {
        ...
        const validateAppointmentTiming = (timing) => {
        ...
        const validationUtils = {
            ...
            validateFullName: (input) => {
                if (!input || typeof input !== 'string')
                    return { isValid: false, error: "Please provide your first and last name." };
                const trimmed = input.trim();
                if (trimmed.length < 2)
                    return { isValid: false, error: "Please provide your first and last name." };
                const nameParts = trimmed.split(' ').filter(part => part.length > 0);
                if (nameParts.length < 2)
                    return { isValid: false, error: "Please provide both your first and last name." };
                const nameRegex = /^[a-zA-Z\s\-']+$/;
                if (!nameRegex.test(trimmed))
                    return { isValid: false, error: "Please provide a valid name using only letters, spaces, hyphens, and apostrophes." };
                const correctedName = nameParts.map(part => part.charAt(0).toUpperCase() + part.slice(1).toLowerCase()).join(' ');
                return { isValid: true, value: correctedName };
            },
            validateDateOfBirth: (input) => {
                if (!input || typeof input !== 'string')
                    return { isValid: false, error: "Please provide your date of birth." };
                const trimmed = input.trim();
                const dateFormats = [
                ...
                if (!isValidFormat)
                    return { isValid: false, error: "Please provide your date of birth in a valid format (e.g., MM/DD/YYYY)." };
            ...
            validatePhone: (input) => {
                if (!input || typeof input !== 'string')
                    return { isValid: false, error: "Please provide your phone number." };
                const trimmed = input.trim();
                const digitsOnly = trimmed.replace(/\D/g, '');
                if (digitsOnly.length < 10)
                    return { isValid: false, error: "Please provide a valid phone number with at least 10 digits." };
            ...
            validateLocation: (input, availableLocations) => {
                ...
                if (availableLocations && availableLocations.length > 0) {
                    ...
                    const matchingLocation = availableLocations.find(loc => loc.toLowerCase() === locationLower ||
                ...
                if (trimmed.length < 2)
                    return { isValid: false, error: "Please provide a valid location." };
                return { isValid: true, value: trimmed };
            },
            validatePainLevel: (input) => {
                if (!input || typeof input !== 'string')
                    return { isValid: false, error: "Please provide your pain level." };
                const trimmed = input.trim();
                const painLevel = parseInt(trimmed);
                if (isNaN(painLevel))
                    return { isValid: false, error: "Please provide your pain level as a number from 1 to 10." };
            ...
            validateSymptoms: (input) => {
                ...
                const trimmed = input.trim();
                if (trimmed.length < 3)
                    return { isValid: false, error: "Please provide a more detailed description of your symptoms." };
                if (trimmed.length > 500)
                    return { isValid: false, error: "Please provide a shorter description of your symptoms." };
                return { isValid: true, value: trimmed };
            },
            validateProcedure: (input, availableProcedures) => {
                if (!input || typeof input !== 'string')
                    return { isValid: false, error: "Please select a procedure." };
                const trimmed = input.trim();
                if (availableProcedures && availableProcedures.length > 0) {
                    const procedureLower = trimmed.toLowerCase();
                    const matchingProcedure = availableProcedures.find(proc => proc.toLowerCase() === procedureLower ||
                ...
                if (trimmed.length < 2)
                    return { isValid: false, error: "Please provide a valid procedure." };
                return { isValid: true, value: trimmed };
            },
            validateInsurance: (input, availableInsurance) => {
                if (!input || typeof input !== 'string')
                    return { isValid: false, error: "Please provide your insurance information." };
                const trimmed = input.trim();
                if (availableInsurance && availableInsurance.length > 0) {
                    const insuranceLower = trimmed.toLowerCase();
                    const matchingInsurance = availableInsurance.find(ins => ins.toLowerCase() === insuranceLower ||
                ...
                if (trimmed.length < 2)
                    return { isValid: false, error: "Please provide valid insurance information." };
                return { isValid: true, value: trimmed };
            },
            validatePolicyHolder: (input) => {
                if (!input || typeof input !== 'string')
                    return { isValid: false, error: "Please provide the Policy Holder's First Name and Last Name." };
                const trimmed = input.trim();
                if (trimmed.length < 2)
                    return { isValid: false, error: "Please provide the Policy Holder's First Name and Last Name." };
                const nameParts = trimmed.split(' ').filter(part => part.length > 0);
                if (nameParts.length < 2)
                    return { isValid: false, error: "Please provide both the Policy Holder's First Name and Last Name." };
                const nameRegex = /^[a-zA-Z\s\-']+$/;
                if (!nameRegex.test(trimmed))
                    return { isValid: false, error: "Please provide a valid name using only letters, spaces, hyphens, and apostrophes." };
                const correctedName = nameParts.map(part => part.charAt(0).toUpperCase() + part.slice(1).toLowerCase()).join(' ');
            ...
            validateInsuranceId: (input) => {
                if (!input || typeof input !== 'string')
                    return { isValid: false, error: "Please provide your insurance ID number." };
                const trimmed = input.trim();
                if (trimmed.length < 3)
                    return { isValid: false, error: "Please provide a valid insurance ID number." };
                if (trimmed.length > 20)
                    return { isValid: false, error: "Insurance ID number seems too long. Please check and try again." };
                return { isValid: true, value: trimmed };
            },
            validateGroupNumber: (input) => {
                if (!input || typeof input !== 'string')
                    return { isValid: false, error: "Please provide your group number." };
                const trimmed = input.trim();
                if (trimmed.length < 2)
                    return { isValid: false, error: "Please provide a valid group number." };
                if (trimmed.length > 15)
                    return { isValid: false, error: "Group number seems too long. Please check and try again." };
                return { isValid: true, value: trimmed };
            },
            validateAdditionalInfo: (input) => {
                ...
                if (trimmed.length > 1000)
                    return { isValid: false, error: "Additional information is too long. Please provide a shorter description." };
                return { isValid: true, value: trimmed };
            }
        };
        // Spell check functionality
        const spellCheck = (input, fieldType) => {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            const demoData = (originalContext === null || originalContext === void 0 ? void 0 : originalContext.demoData) || data;
            const config = (originalContext === null || originalContext === void 0 ? void 0 : originalContext.config) || data.config || {};
            const webhookData = (originalContext === null || originalContext === void 0 ? void 0 : originalContext.webhookData) || {};
            const fullContext = (originalContext === null || originalContext === void 0 ? void 0 : originalContext.fullContext) || {};
            ...
            const openingHours = locations.map((loc) => {
                var _a;
                return ({
                    name: loc.name || loc.city,
                    hours: loc.hours || ((_a = loc.opening_hours) === null || _a === void 0 ? void 0 : _a.weekday_text) || []
                });
            }).filter((loc) => loc.hours && loc.hours.length > 0);
            // Helper function to get available days for a location
            const getAvailableDays = (locationName) => {
                // Get business hours from the demo data or config
                const businessHours = (config === null || config === void 0 ? void 0 : config.businessHours) || (demoData === null || demoData === void 0 ? void 0 : demoData.businessHours) || {};
                const locations = (config === null || config === void 0 ? void 0 : config.locations) || (demoData === null || demoData === void 0 ? void 0 : demoData.locations) || [];
                // Find the specific location
                const location = locations.find((loc) => {
                    var _a, _b;
                    return ((_a = loc.name) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(locationName.toLowerCase())) ||
                        locationName.toLowerCase().includes((_b = loc.name) === null || _b === void 0 ? void 0 : _b.toLowerCase());
                });
                if (location && location.hours && Array.isArray(location.hours)) {
                    ...
                    const availableDays = [];
                    location.hours.forEach((dayInfo) => {
            ...
            const getBusinessHours = (locationName) => {
                const location = openingHours.find((loc) => loc.name.toLowerCase().includes(locationName.toLowerCase()) ||
            ...
            const getBusinessHoursForDay = (dayName, locations) => {
                for (const location of locations) {
                    if (location.hours && Array.isArray(location.hours)) {
                        const dayInfo = location.hours.find((day) => day.toLowerCase().startsWith(dayName.toLowerCase()));
            ...
            if (config.services && Array.isArray(config.services)) {
                ...
                serviceNames = config.services.map((service) => {
            ...
            if (serviceNames.length === 0 && demoData.demo_services && Array.isArray(demoData.demo_services)) {
                serviceNames = demoData.demo_services.map((service) => service.service_name || service.name).filter(Boolean);
            ...
            if (config.insurance_providers && Array.isArray(config.insurance_providers)) {
                insuranceProviders = config.insurance_providers.map((provider) => provider.name || provider).filter(Boolean);
            ...
            if (insuranceProviders.length === 0 && demoData.insurance_providers && Array.isArray(demoData.insurance_providers)) {
                insuranceProviders = demoData.insurance_providers.map((provider) => provider.name || provider).filter(Boolean);
            ...
            let currentBookingState = data.bookingState || webhookData.bookingState || ((_a = originalContext === null || originalContext === void 0 ? void 0 : originalContext.webhookData) === null || _a === void 0 ? void 0 : _a.bookingState) || "initial";
            ...
            const bookingDataSource = data.bookingData || webhookData.bookingData || ((_b = originalContext === null || originalContext === void 0 ? void 0 : originalContext.webhookData) === null || _b === void 0 ? void 0 : _b.bookingData) || "{}";
            ...
            console.log("🔧 DEBUG - originalContext?.webhookData?.bookingData:", (_c = originalContext === null || originalContext === void 0 ? void 0 : originalContext.webhookData) === null || _c === void 0 ? void 0 : _c.bookingData);
            ...
            switch (currentBookingState) {
                ...
                case "collecting_day_of_week":
                    ...
                    const allLocations = (config === null || config === void 0 ? void 0 : config.locations) || (demoData === null || demoData === void 0 ? void 0 : demoData.locations) || [];
                    const allAvailableDays = new Set();
                    // DEBUG: Log what data is available
                    console.log("🔧 DEBUG - Config locations:", config === null || config === void 0 ? void 0 : config.locations);
                    console.log("🔧 DEBUG - Demo data locations:", demoData === null || demoData === void 0 ? void 0 : demoData.locations);
                    ...
                    const availableDays = Array.from(allAvailableDays).sort((a, b) => dayOrder.indexOf(a) - dayOrder.indexOf(b));
                ...
                case "collecting_additional_info":
                    ...
                    const highlightIfMissing = (value, fieldName) => {
            ...
            const preservedContext = {
                demoData: (originalContext === null || originalContext === void 0 ? void 0 : originalContext.demoData) || data,
