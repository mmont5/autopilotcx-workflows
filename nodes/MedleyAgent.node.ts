...
export class MedleyAgent implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		function extractSymptomsAndBodyParts(text: string): string | null {
		...
		function getEmpatheticResponse(symptom: string): string {
			const template = responseTemplates[Math.floor(Math.random() * responseTemplates.length)];
			return template.replace("{symptom}", symptom);
		}

		function getIndustrySpecificWelcome(agentName: string, companyName: string, industry: string): string {
		...
		for (let i = 0; i < items.length; i++) {
			...
			const companyName = (data.company && typeof data.company === 'object' && 'name' in data.company) ? (data.company as any).name : 
			...
			const industry = (typeof data.industry === 'string' ? data.industry : 
							(data.company && typeof data.company === 'object' && 'industry' in data.company) ? (data.company as any).industry : "healthcare");
			const category = (typeof data.category === 'string' ? data.category : 
							(data.company && typeof data.company === 'object' && 'category' in data.company) ? (data.company as any).category : "");
			const specialists = Array.isArray(data.specialists) ? data.specialists : [];
			const locations = Array.isArray(data.locations) ? data.locations : [];
			const contactPhone = (typeof data.contact === 'object' && data.contact && 'phone' in data.contact) ? (data.contact as any).phone : 
								(typeof data.company_phone === 'string' ? data.company_phone : "");

			// Safe type guards for arrays
			...
			const userContext = data.userContext || data.context || {};
			const conversationStage = data.conversationStage || "initial";

			// Extract symptoms from user message
			const extractedSymptoms = extractSymptomsAndBodyParts(userMessage);
			const symptom = extractedSymptoms || "health concern";

			// Safe type guards for user message
			const safeUserMessage = typeof userMessage === 'string' ? userMessage : '';
			// Ensure locations is always an array for .map usage
			const safeLocations = Array.isArray(locations) ? locations : [];

			let agentResponse = "";
			// CRITICAL FIX: Preserve suggestedActions from input instead of overriding
			let suggestedActions: any[] = data.suggestedActions || [];
			let llm_used = false;
			...
			if (isWelcomeMessage) {
				...
				// CRITICAL FIX: Only set buttons if none exist from input
				...
				try {
					...
					const businessContext = {
						...
						conversation: {
							...
							previousInteractions: (userContext as any).previousMessages || []
						},
						industryKnowledge: {
							healthcare: {
								...
								locations: safeLocations.map((loc: any) => `${String(loc.name ?? '')}: ${String(loc.address1 ?? '')}, ${String(loc.city ?? '')}, ${String(loc.state ?? '')}`)
							}
						}
					};
...
INDUSTRY: ${industry || ''}
CATEGORY: ${category || ''}
SPECIALISTS: ${safeSpecialists.map((s: any) => s.name || s).join(", ")}
LOCATIONS: ${safeLocations.map((loc: any) => `${String(loc.name ?? '')}: ${String(loc.address1 ?? '')}, ${String(loc.city ?? '')}, ${String(loc.state ?? '')}`).join('; ')}
...
Respond as a real human employee would - with expertise, empathy, and genuine care for helping this person.`,
					...
					if (response.ok) {
						...
						if (llmData && typeof llmData.text === 'string' && llmData.text.trim()) {
							...
							// CRITICAL FIX: Preserve existing buttons, only add LLM buttons if none exist
							if (suggestedActions.length === 0) {
				...
				if (!llm_used) {
					...
					if (isEmergency) {
						...
						// CRITICAL FIX: Only set emergency buttons if no buttons exist
						if (suggestedActions.length === 0) {
							suggestedActions = [
						...
						if (extractedSymptoms && extractedSymptoms !== 'pain') {
							...
							// CRITICAL FIX: Only set fallback buttons if no buttons exist
							if (suggestedActions.length === 0) {
								suggestedActions = [
									{ text: "Book Appointment", action: "I want to book an appointment" },
			...
			console.log("🚀 MEDLEYAGENT: Final suggestedActions:", suggestedActions);
