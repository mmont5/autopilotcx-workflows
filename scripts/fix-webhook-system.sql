-- Fix Webhook System - Complete Cleanup and Setup
-- Run this in your Supabase SQL Editor to fix all webhook issues

-- Step 1: Drop ALL existing webhook-related triggers and functions
...
DROP FUNCTION IF EXISTS update_webhook_url(TEXT, TEXT);
DROP FUNCTION IF EXISTS get_webhook_url(TEXT);
DROP FUNCTION IF EXISTS test_webhook(TEXT);

-- Step 2: Create the webhook configuration table (if it doesn't exist)
...
-- Step 3: Create the main webhook notification function with current NGROK URL
...
BEGIN
    -- Get the webhook URL from configuration table
...
-- Step 4: Create function to update webhook URL
...
-- Step 5: Create function to get webhook URL
...
-- Step 7: Create function to test webhook
...
-- Step 8: Create the trigger (this is the ONLY trigger that should exist)
CREATE TRIGGER demo_created_webhook
    AFTER INSERT ON demo
    FOR EACH ROW
    EXECUTE FUNCTION notify_demo_created();

-- Step 9: Insert the current NGROK webhook URL
INSERT INTO webhook_config (name, url, is_active)
VALUES (
    'demo_created', 
    'https://d344-2603-9001-9300-481f-5ce6-750d-cc80-ff3b.ngrok-free.app/webhook/0a274e64-8902-4f73-ac54-7f37206c7a31',
    true
)
ON CONFLICT (name) 
DO UPDATE SET 
...
-- Step 13: Show current configuration
SELECT 
    ...
    updated_at
FROM webhook_config 
WHERE name = 'demo_created';

-- Step 14: Show all triggers on demo table
SELECT 
    'Demo Table Triggers' as info,
    trigger_name,
    event_manipulation,
    action_statement
FROM information_schema.triggers 
WHERE event_object_table = 'demo';

-- Step 15: Show all webhook functions
SELECT 
    'Webhook Functions' as info,
    proname as function_name,
    prorettype::regtype as return_type
FROM pg_proc 
WHERE proname IN ('notify_demo_created', 'trigger_demo_webhook', 'update_webhook_url', 'get_webhook_url', 'test_webhook')
ORDER BY proname; 